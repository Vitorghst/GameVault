...
const DashboardRAWG = () => {
  ...
  const loadCollection = async () => {
    try {
      ...
      const newStats = response.data.reduce((acc, game) => {
        acc.total++;
  ...
  return (
    <>
      ...
      <div className="hero-search">
        <div className="hero-content">
          ...
          <div className="search-trends">
            <span>Populares:</span>
            <button onClick={() => setSearchTerm('The Witcher')}>The Witcher</button>
            <button onClick={() => setSearchTerm('Zelda')}>Zelda</button>
            <button onClick={() => setSearchTerm('God of War')}>God of War</button>
            <button onClick={() => setSearchTerm('Red Dead')}>Red Dead</button>
            <button onClick={() => setSearchTerm('Elden Ring')}>Elden Ring</button>
          </div>
        </div>
      </div>
      ...
      <div className="dashboard-tabs">
        ...
        >
          <span className='logo-explorar'><FaMagnifyingGlass /></span> Explorar {searchTerm && `"${searchTerm}"`}
        </button>
      </div>
      ...
      <div className="main-content-rawg">
          ...
          <div className="explorar-section">
            {searchResults.length > 0 ? (
              <>
                <div className="section-header">
                  <div>
                    ...
                    <span className="results-count">{searchResults.length} jogos encontrados</span>
                  </div>
                  <button className="suggested-user-follow-btn" onClick={clearSearch}>
              ...
              <div className="explorar-default">
                {/* Seção de populares */}
                <div className="section-header">
                  <h2><FaFire style={{ color: '#ff6b6b' }} /> Jogos Populares</h2>
                  <span className="section-subtitle">Baseado na comunidade</span>
                </div>
                <div className="games-grid-rawg">
                ...
                <div className="section-header" style={{ marginTop: '48px' }}>
                  <h2><FaStar style={{ color: '#ffd700' }} /> Novos Lançamentos</h2>
        ...
        {activeTab === 'colecao' && (
          <div className="colecao-section">
            <div className="section-header">
              <h2> <span className="logo-collection"><BsFillCollectionFill /></span> Minha Coleção</h2>
              <div className="filters-rawg">
                ...
                >
                  <span className='logo-jogando'><IoLogoGameControllerB /></span> Jogando ({collection.filter(g => g.status === 'jogando').length})
                </button>
                <button
                  className={`filter-chip ${activeFilter === 'zerado' ? 'active' : ''}`}
                  onClick={() => filterCollection('zerado')}
                >
                  <span className='logo-finalizado'><RiVerifiedBadgeFill /></span> Zerados ({collection.filter(g => g.status === 'zerado').length})
                </button>
                <button
                  className={`filter-chip ${activeFilter === 'favorito' ? 'active' : ''}`}
                  onClick={() => filterCollection('favorito')}
                >
                  <span className='logo-favoritos'><FaHeart /></span> Favoritos ({collection.filter(g => g.status === 'favorito').length})
                </button>
                <button
                  className={`filter-chip ${activeFilter === 'quero jogar' ? 'active' : ''}`}
                  onClick={() => filterCollection('quero jogar')}
                >
                  <span className='logo-quero'><FaHourglassHalf /></span> Quero Jogar ({collection.filter(g => g.status === 'quero jogar').length})
                </button>
              </div>
            </div>
            ...
            {filteredCollection.length === 0 ? (
              <div className="empty-collection">
                ...
                <p>Busque jogos na aba Explorar e comece a construir sua biblioteca!</p>
                <button className="explorar-btn" onClick={() => setActiveTab('explorar')}>
                  Explorar Jogos
                </button>
              </div>
            ) : (
